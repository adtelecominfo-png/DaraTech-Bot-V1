...
// ── Sudo cache ────────────────────────────────────────────────────────────────
// Avoid reading the file on every single message by caching the list in memory.
let _sudoCache = null;
let _sudoCacheTime = 0;
const SUDO_CACHE_TTL = 30000; // refresh at most every 30 s
...
function _getSudoCached() {
    ...
    if (!_sudoCache || now - _sudoCacheTime > SUDO_CACHE_TTL) {
        try {
            const data = loadUserGroupData();
            _sudoCache = Array.isArray(data.sudo) ? data.sudo : [];
        } catch { _sudoCache = []; }
        _sudoCacheTime = now;
    }
    return _sudoCache;
...
async function isSudo(userId) {
    try {
        ...
        const cleanUser = _cleanJid(userId);
        return list.some(s => {
            if (s === userId) return true;
            const cleanS = _cleanJid(s);
            if (cleanS === cleanUser) return true;
            // Suffix match: handles country-code mismatches
            // e.g. stored "2348100785677" matches incoming "8100785677" and vice-versa
            if (cleanS.length >= 7 && cleanUser.length >= 7) {
                if (cleanUser.endsWith(cleanS) || cleanS.endsWith(cleanUser)) return true;
            }
            return false;
        });
    } catch (error) {
...
async function addSudo(userJid) {
    try {
        ...
        // Normalise before storing so the list stays consistent
        const cleanNew = _cleanJid(userJid);
        const already = data.sudo.some(s => s === userJid || _cleanJid(s) === cleanNew);
        if (!already) {
...
async function removeSudo(userJid) {
    try {
        ...
        const data = loadUserGroupData();
        if (!data.sudo) data.sudo = [];
        const cleanTarget = _cleanJid(userJid);
        const before = data.sudo.length;
        data.sudo = data.sudo.filter(s => s !== userJid && _cleanJid(s) !== cleanTarget);
        if (data.sudo.length !== before) saveUserGroupData(data);
        return true;
    } catch (error) {
