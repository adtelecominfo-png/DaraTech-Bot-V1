...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    ...
    // Normalize to phone number only (strip :device suffix and @domain) for robust comparison
    const botPhone = rawBotId.split(':')[0].split('@')[0];
    const bot = groupMetadata.participants.find(p => p.id.split(':')[0].split('@')[0] === botPhone);
    if (!bot?.admin) {
