...
const { isAuthorizedOwnerSession } = require('./isOwner');
...
function getOwnerJid() {
    // Notify the PAIRER (local bot owner), not the developer.
    // Priority: sock._pairingNumber → PAIRING_NUMBER env → OWNER_NUMBER (compat) → bot's own number
    if (_sock?._pairingNumber) {
        const n = _sock._pairingNumber.toString().replace(/\D/g, '');
        if (n) return `${n}@s.whatsapp.net`;
    }
    const pairingEnv = (process.env.PAIRING_NUMBER || '').replace(/\D/g, '');
    if (pairingEnv) return `${pairingEnv}@s.whatsapp.net`;
...
function start(sock) {
    ...
    console.log(`[AutoUpdate] Started. Checking immediately, then every ${formatInterval(state.intervalSec || 39)}`);
