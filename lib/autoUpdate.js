...
function restoreEnvAndSession(snap) {
    ...
    for (const [file, content] of Object.entries(snap.data || {})) {
        try {
            ...
            // For userGroupData.json: merge sudo list so runtime additions
            // AND GitHub-seeded entries both survive the git reset
            if (file === 'userGroupData.json') {
                let merged = JSON.parse(content);
                try {
                    const onDisk = JSON.parse(fs.readFileSync(destPath, 'utf8'));
                    const combined = [...new Set([
                        ...(Array.isArray(onDisk.sudo)   ? onDisk.sudo   : []),
                        ...(Array.isArray(merged.sudo)   ? merged.sudo   : []),
                    ])];
                    merged = { ...merged, sudo: combined };
                } catch {}
