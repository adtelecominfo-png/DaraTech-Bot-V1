...
// ─── State ────────────────────────────────────────────────────────────────────
// interval is stored in SECONDS internally; default 6 hours
function readState() {
    try { return JSON.parse(fs.readFileSync(STATE_FILE, 'utf8')); }
    catch { return { enabled: true, intervalSec: 21600 }; }
}
...
function scheduleNext() {
    const state = readState();
    if (!state.enabled) return;
    const sec = Math.max(300, state.intervalSec || 21600); // minimum 5 minutes
    const ms  = sec * 1000;
